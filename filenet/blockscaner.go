...
//FNBlockScanner ontology的区块链扫描器
type FNBlockScanner struct {
...
//NewFNBlockScanner 创建区块链扫描器
func NewFNBlockScanner(wm *WalletManager) *FNBlockScanner {
	bs := FNBlockScanner{
...
func (bs *FNBlockScanner) SetRescanBlockHeight(height uint64) error {
	height = height - 1
	if height < 0 {
...
//ScanBlockTask 扫描任务
func (bs *FNBlockScanner) ScanBlockTask() {
...
//ScanBlock 扫描指定高度区块
func (bs *FNBlockScanner) ScanBlock(height uint64) error {
...
func (bs *FNBlockScanner) scanBlock(height uint64) (*Block, error) {
...
func (bs *FNBlockScanner) ScanTxMemPool() {
...
func (bs *FNBlockScanner) RescanFailedRecord() {
...
func (bs *FNBlockScanner) newBlockNotify(block *Block, isFork bool) {
...
func (bs *FNBlockScanner) BatchExtractTransaction(blockHeight uint64, blockHash string, txs []string, memPool bool) error {
...
func (bs *FNBlockScanner) extractRuntime(producer chan ExtractResult, worker chan ExtractResult, quit chan struct{}) {
...
func (bs *FNBlockScanner) ExtractTransaction(blockHeight uint64, blockHash string, txid string, scanAddressFunc openwallet.BlockScanAddressFunc, memPool bool) ExtractResult {
...
func (bs *FNBlockScanner) extractTransaction(trx *Transaction, result *ExtractResult, scanAddressFunc openwallet.BlockScanAddressFunc) {
	var (
		success = true
	)
	...
	if trx == nil {
		...
		index := uint64(0)
		for _, txTo := range trx.To {
			sourceKey, ok = scanAddressFunc(txTo.Address)
			if ok {
				output := openwallet.TxOutPut{}
				output.TxID = trx.TxID
				output.Address = txTo.Address
				output.Amount = convertToAmount(txTo.Amount)
				...
				output.Index = index
				index ++
				output.Sid = openwallet.GenTxOutPutSID(trx.TxID, bs.wm.Symbol(), "", index)
		...
		for _, extractData := range result.extractData {
			to := []string{}
			for _, txTo := range trx.To {
				to = append(to, txTo.Address + ":" + convertToAmount(txTo.Amount))
			}
...
func (bs *FNBlockScanner) newExtractDataNotify(height uint64, extractData map[string]*openwallet.TxExtractData) error {
...
//func (bs *FNBlockScanner) SaveRechargeToWalletDB(height uint64, list []*openwallet.Recharge) error {
//
//	for _, r := range list {
//
...
func (bs *FNBlockScanner) GetCurrentBlockHeader() (*openwallet.BlockHeader, error) {
	var (
...
func (bs *FNBlockScanner) GetScannedBlockHeader() (*openwallet.BlockHeader, error) {
	...
	if blockHeight == 0 {
		blockHeight, err = bs.wm.GetBlockHeight()
		if err != nil {
			bs.wm.Log.Errorf("FN GetBlockHeight failed,err = %v", err)
			return nil, err
		}
...
func (bs *FNBlockScanner) GetScannedBlockHeight() uint64 {
	localHeight, _, _ := bs.wm.GetLocalNewBlock()
	return localHeight
}

func (bs *FNBlockScanner) ExtractTransactionData(txid string, scanTargetFunc openwallet.BlockScanTargetFunc) (map[string][]*openwallet.TxExtractData, error) {
...
//func (bs *FNBlockScanner) DropRechargeRecords(accountID string) error {
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
//func (bs *FNBlockScanner) DeleteRechargesByHeight(height uint64) error {
//
//	bs.mu.RLock()
//	defer bs.mu.RUnlock()
//
...
func (bs *FNBlockScanner) SaveUnscanRecord(record *UnscanRecord) error {
...
func (bs *FNBlockScanner) GetSourceKeyByAddress(address string) (string, bool) {
	bs.Mu.RLock()
	defer bs.Mu.RUnlock()
...
// func (bs *FNBlockScanner) GetWalletByAddress(address string) (*openwallet.Wallet, bool) {
// 	bs.mu.RLock()
// 	defer bs.mu.RUnlock()
...
func (wm *WalletManager) GetTransaction(txid string) (*Transaction, error) {
	return NewTransaction(txid), nil
}
...
func (bs *FNBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
...
func (bs *FNBlockScanner) GetTransactionsByAddress(offset, limit int, coin openwallet.Coin, address ...string) ([]*openwallet.TxExtractData, error) {
...
//Run 运行
func (bs *FNBlockScanner) Run() error {
...
////Stop 停止扫描
func (bs *FNBlockScanner) Stop() error {
...
//Pause 暂停扫描
func (bs *FNBlockScanner) Pause() error {
...
//Restart 继续扫描
func (bs *FNBlockScanner) Restart() error {
...
func (bs *FNBlockScanner) setupSocketIO() error {
